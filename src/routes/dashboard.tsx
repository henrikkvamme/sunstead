...
function detailsForNode(
  node: GraphNode,
  nodes: GraphNode[] = graphNodes,
  edges: GraphEdge[] = graphEdges,
  detailsById: NodeDetails = nodeDetails,
): NodeDetails[string] {
  const directDetails = detailsById[node.id];
  ...
  const connectedSourceIds = edges
    ...
    .filter((id) => nodes.some((candidate) => candidate.id === id && candidate.kind === "source"));
  const connectedSources = connectedSourceIds.flatMap(
    (sourceId) => detailsById[sourceId]?.sources ?? [],
  );
...
export function Dashboard() {
  ...
  const [platformGraph, setPlatformGraph] = useState<PlatformGraphData | null>(null);
  const commandInputRef = useRef<HTMLInputElement>(null);
  const searchButtonRef = useRef<HTMLButtonElement>(null);
  const graphMode: GraphMode = viewMode === "overview" ? "overview" : "focused";

  const dashboardNodes = useMemo(
    () => (platformGraph ? [...graphNodes, ...platformGraph.nodes] : graphNodes),
    [platformGraph],
  );
  const dashboardEdges = useMemo(
    () => (platformGraph ? [...graphEdges, ...platformGraph.edges] : graphEdges),
    [platformGraph],
  );
  const dashboardNodeDetails = useMemo(
    () => (platformGraph ? { ...nodeDetails, ...platformGraph.details } : nodeDetails),
    [platformGraph],
  );
  const dashboardMedicineSupplyChainEdges = useMemo(
    () =>
      platformGraph
        ? {
            ...medicineSupplyChainEdges,
            ...platformGraph.medicineSupplyChainEdges,
          }
        : medicineSupplyChainEdges,
    [platformGraph],
  );
  ...
  const selectedNode =
    dashboardNodes.find((node) => node.id === selectedNodeId) ?? dashboardNodes[0];
  const selectedDetails = detailsForNode(
    selectedNode,
    dashboardNodes,
    dashboardEdges,
    dashboardNodeDetails,
  );
  ...
  useEffect(() => {
    ...
    async function loadPlatformGraph() {
      try {
        const response = await fetch(platformGraphSnapshotUrl);

        if (!response.ok) {
          return;
        }

        const snapshot = (await response.json()) as PlatformGraphSnapshot;

        if (mounted) {
          setPlatformGraph(buildPlatformGraphData(snapshot));
        }
      } catch {
        if (mounted) {
          setPlatformGraph(null);
        }
      }
    }

    void loadPlatformGraph();
    ...
    <main className={cn("medicine-graph-screen", `is-${graphMode}`, `view-${viewMode}`)}>
        ...
        <div className="medicine-graph-nav-actions">
          ...
          {platformGraph ? (
            <span className="medicine-graph-risk-pill">
              ...
              {platformGraph.nodes.length} Neo4j nodes
            </span>
          ) : null}
          <button
      ...
      <section className="medicine-graph-stage" aria-label="Medicine Risk Network">
        <MedicineRiskGraph
          ...
          medicineSupplyChainEdges={dashboardMedicineSupplyChainEdges}
          mode={graphMode}
...
function MedicineRiskGraph({
  ...
  const selectedNeighborEdgeIds = new Set<string>();

  for (const edge of edges) {
    if (edge.from === selectedNodeId || edge.to === selectedNodeId) {
      selectedNeighborEdgeIds.add(edge.id);
    }
  }
  ...
  const visibleEdges =
    mode === "overview"
      ? edges.filter((edge) => !edge.scripted || addedEvidence)
      : edges.filter((edge) => {
          ...
          return (
            ...
            selectedNeighborEdgeIds.has(edge.id) ||
            Boolean(edge.actionPath)
          );
        });
  ...
  const actionEdgeIds = new Set(edges.filter((edge) => edge.actionPath).map((edge) => edge.id));
  ...
  return (
    ...
    >
      ...
      {visibleNodes.map((node, index) => {
        ...
        const isPlatformNode = isPlatformNodeId(node.id);
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isPlatformNode && "is-platform-node",
              isSource && "is-evidence-satellite",
              mode === "overview" &&
                (node.kind !== "medicine" || isPlatformNode) &&
                "is-overview-icon",
            )}
            key={`${mode}-${node.id}`}
            onClick={() => {
...
function graphChatStatsLabel(payload: GraphChatApiResponse) {
  const metadata = payload.audit.metadata ?? {};
  const snapshotMode = metadata.snapshotMode
    ? metadata.snapshotMode.replaceAll("_", " ")
    : "curated graph";
  const sourceGraphNodes = metadata.sourceGraphNodes ?? payload.graphStats.sourceGraphNodes;
  const sourceGraphRelationships =
    metadata.sourceGraphRelationships ?? payload.graphStats.sourceGraphRelationships;
  const sourceCounts =
    sourceGraphNodes !== undefined && sourceGraphRelationships !== undefined
      ? `${sourceGraphNodes} source nodes, ${sourceGraphRelationships} source edges`
      : `${payload.graphStats.platformNodes} platform nodes, ${payload.graphStats.platformEdges} platform edges`;

  return `${payload.graphStats.nodes} visible nodes, ${payload.graphStats.edges} visible edges | ${snapshotMode} | ${sourceCounts} | audit ${payload.audit.auditId.slice(0, 8)}`;
}
...
function RiskSidePanel({
  ...
  const submitGraphQuestion = async (question: string) => {
    const trimmed = question.trim();

    if (!trimmed || chatState === "running") {
      return;
    }
    ...
    try {
      const response = await fetch("/api/graph-chat", {
        body: JSON.stringify({ nodeId: node.id, question: trimmed }),
        headers: { "content-type": "application/json" },
        method: "POST",
      });
      const graphChatResponse = (await response.json()) as Partial<GraphChatApiResponse> & {
        error?: string;
      };

      if (!response.ok || !graphChatResponse.answer || !graphChatResponse.graphStats) {
        throw new Error(graphChatResponse.error || "Graph answer failed.");
      }

      const payload = graphChatResponse as GraphChatApiResponse;
      ...
      setChatMessages((messages) => [
        ...messages,
        {
          ...
          stats: graphChatStatsLabel(payload),
          text: payload.answer,
        },
      ]);
    } catch (error) {
      setChatError(error instanceof Error ? error.message : "Graph answer failed.");
    } finally {
      setChatState("idle");
    }
  };

  const handleGraphQuestionSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    void submitGraphQuestion(chatQuestion);
  };
      ...
      <section className="risk-panel-section">
        ...
        <div className="evidence-list">
          {details.sources.map((source, index) => (
            <article key={`${source.title}-${source.meta}-${index}`}>
        ...
        <div className="prompt-list">
          {details.prompts.map((prompt) => (
            <button
              ...
              type="button"
              onClick={() => {
                if (prompt === "Find newer evidence") {
                  onStartInvestigation();
                  return;
                }

                void submitGraphQuestion(prompt);
              }}
            >
        ...
        <div className="graph-chat-thread" aria-live="polite">
          {chatMessages.length ? (
            chatMessages.map((message) => (
              <article className={cn("graph-chat-message", `is-${message.role}`)} key={message.id}>
                <strong>{message.role === "user" ? "You" : "Graph agent"}</strong>
                <p>{message.text}</p>
                {message.stats ? <span>{message.stats}</span> : null}
                {message.sources?.length ? (
                  <ul>
                    {message.sources.map((source, index) => (
                      <li key={`${message.id}-${source.title}-${source.meta}-${index}`}>
                        {source.title} <span>{source.meta}</span>
                      </li>
                    ))}
                  </ul>
                ) : null}
            ...
            <article className="graph-chat-message is-assistant">
              <strong>Graph agent</strong>
              <p>Ask about this node, its evidence, connected risks, or source provenance.</p>
            </article>
          )}
        </div>
        {chatError ? <p className="graph-chat-error">{chatError}</p> : null}
        <form
          className="agent-input"
          aria-label="Graph investigation input"
          onSubmit={handleGraphQuestionSubmit}
        >
          <Bot aria-hidden size={15} />
          <input
            aria-label="Ask about the selected graph node"
            disabled={chatState === "running"}
            placeholder="Ask about this graph node"
            value={chatQuestion}
            onChange={(event) => setChatQuestion(event.target.value)}
          />
          <button
            aria-label="Ask graph"
            disabled={chatState === "running" || !chatQuestion.trim()}
            type="submit"
          >
            {chatState === "running" ? (
