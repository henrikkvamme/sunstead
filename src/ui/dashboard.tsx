        ...
        {navSections.map((section) => (
          <div className="app-dashboard-nav-section" key={section.label ?? "primary"}>
            ...
            {section.items.map(({ badge, hasChevron, href, icon: Icon, isActive, label, meta }) => (
              <a className={cn(isActive && "active")} href={href} key={`${section.label ?? "primary"}-${label}`}>
...
export function AppDashboardIconButton({
  ...
  return (
    <button
      aria-label={ariaLabel}
      className={cn("app-dashboard-icon-button", badge && "has-badge", desktopOnly && "desktop-only")}
      type="button"
    >
      {children}
...
export function AppDashboardAction({
  ...
  return (
    <a className={isPrimary ? "app-dashboard-upgrade-button desktop-only" : "app-dashboard-utility-button desktop-only"} href={href}>
      {icon}
      <span>{children}</span>
    </a>
  );
...
export function AppDashboardColumn({ children, side }: { children: ReactNode; side: "left" | "right" }) {
...
export function DashboardTokenPanel({
  href,
  token,
}: {
  href: string;
