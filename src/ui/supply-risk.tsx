...
export function FullScreenSupplyRiskGraph({
  ...
  const renderLinks = (coordinateSet: "desktop" | "mobile") =>
    edges.map((edge) => {
      ...
      return (
        <path
          className={cn("fullscreen-risk-link", `risk-${edge.level}`)}
          d={curve}
          key={`${coordinateSet}-${edge.from}-${edge.to}`}
        />
      );
    });
  ...
  return (
    <div
      className="fullscreen-risk-graph"
      aria-label="Full-screen medicine supply chain risk graph"
    >
      <svg
        aria-hidden
        className="fullscreen-risk-links"
        viewBox="0 0 100 100"
        preserveAspectRatio="none"
      >
        <defs>
